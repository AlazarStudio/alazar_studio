...
export const CaseHomeList = (props) => (
  <List {...props}>
    <Datagrid rowClick="edit">
      ...
      {/* Исправляем поле для отображения разработчика */}
      <TextField
  source="developers"
  label="Разработчики"
  render={(record) => {
    // Проверяем, есть ли разработчики в записи
    if (record.developers) {
      // Извлекаем имена всех разработчиков и соединяем их через запятую
      return record.developers.map(developer => developer.name).join(', ');
    }
    return 'Нет разработчиков'; // Если разработчиков нет
  }}
/>

      {/* Исправляем вывод категорий */}
      <TextField
        source="categories"
        label="Категории"
        render={(record) =>
