...
export default function HomePage() {
  ...
  const { categoryId } = useParams(); // Извлекаем параметр categoryId из URL
  ...
  // Фильтруем кейсы по выбранной категории, если она указана
  const filteredCaseHomes = categoryId
    ? caseHomes.filter((caseHome) =>
        caseHome.categories.some((category) => category.id === parseInt(categoryId))
      )
    : caseHomes;
        ...
        <div className={classes.categoryMenu}>
          {categories.map((cat) => (
            <span
              ...
              onClick={() => navigate(`/cases/${cat.id}`)} // Перенаправляем на страницу с кейсами выбранной категории
            >
              {cat.title}
            </span>
          ))}
