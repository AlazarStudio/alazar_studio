...
export default function HomePage() {
  ...
  // Фильтруем кейсы по выбранной категории по title, учитывая, что title может быть в разных регистрах
  const filteredCaseHomes = categoryTitle
    ? caseHomes.filter((caseHome) =>
        caseHome.categories.some(
          (category) => category.title.toLowerCase() === categoryTitle.toLowerCase() // Приводим к нижнему регистру при фильтрации
        )
      )
    : caseHomes;
        ...
        <div className={classes.categoryMenu}>
          {categories.map((cat) => (
            <span
              ...
              onClick={() => navigate(`/cases/${cat.title.toLowerCase()}`)} // Навигация с категорией в URL с приведением в нижний регистр
            >
              {cat.title}
            </span>
          ))}
