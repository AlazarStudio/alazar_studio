...
export const CaseHomeList = (props) => (
  <List {...props}>
    <Datagrid rowClick="edit">
      ...
      {/* Логирование данных разработчиков и категорий */}
      <TextField
        source="developers"
        label="Разработчики"
        render={(record) => {
          // Логируем имена разработчиков
          if (record.developers) {
            console.log('Разработчики:', record.developers.map(developer => developer.name).join(', '));
          }
          return (
            record.developers && record.developers.length > 0
              ? record.developers.map((developer) => developer.name).join(', ')
              : 'Нет разработчиков'
          );
        }}
      />
      ...
      <TextField
        ...
        label="Категории"
        render={(record) => {
          // Логируем названия категорий
          if (record.categories) {
            console.log('Категории:', record.categories.map(category => category.title).join(', '));
          }
          return (
            record.categories && record.categories.length > 0
              ? record.categories.map((category) => category.title).join(', ')
              : 'Нет категорий'
          );
        }}
      />
