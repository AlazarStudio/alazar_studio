...
export default function HomePage() {
  ...
  const { categoryTitle } = useParams(); // Получаем категорию из URL

  // Загружаем данные о кейсах и категориях
  useEffect(() => {
  ...
  // Фильтруем кейсы, чтобы показать только те, которые относятся к выбранной категории
  const filteredCaseHomes = categoryTitle
    ? caseHomes.filter((caseHome) =>
        caseHome.categories.some(
          (category) => transliterate(category.title.toLowerCase()) === categoryTitle.toLowerCase() // Применяем транслитерацию
        )
      )
    : caseHomes;
        ...
        <div className={classes.categoryMenu}>
          {categories.map((cat) => (
            <span
              ...
              onClick={() => navigate(`/${transliterate(cat.title.toLowerCase())}`)} // Навигация с категорий в URL, используя транслитерацию
            >
              {cat.title}
            </span>
          ))}
